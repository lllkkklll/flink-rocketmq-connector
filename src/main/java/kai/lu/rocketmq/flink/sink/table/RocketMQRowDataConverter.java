...
import static kai.lu.rocketmq.flink.sink.table.RocketMQDynamicTableSink.WritableMetadata;
import static kai.lu.rocketmq.flink.sink.table.RocketMQDynamicTableSink.WritableMetadata.KEYS;
import static kai.lu.rocketmq.flink.sink.table.RocketMQDynamicTableSink.WritableMetadata.TAGS;
    ...
    public void open() {
        ...
        if (isDynamicTag && dynamicColumn != null) {
            ...
            checkState(
                    fieldIndex >= 0,
                    String.format(
                            "[MetaQConverter] Could not find the tag column: %s.",
                            dynamicColumn
                    )
            );
    ...
    public Message convert(RowData row) {
        ...
        if (onlyVarbinary) {
            ...
            for (int index = 0; index < bodyFieldIndexes.length; index++) {
                values[index] = RowData.createFieldGetter(
